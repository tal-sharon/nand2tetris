...
class CodeWriter:
    ...
    def _compare_signs(self, cond: str, end_lbl: str, equal_sign_lbl: str) -> None:
        ...
        self._output_stream.write("\n// if R13 == R14: goto EQUAL_SIGN label\n")
        self._output_stream.write("@R13\n"
                                  "D=M\n"
                                  "@R14\n"
                                  "D=D-M\n")
        ...
        self._output_stream.write("\n// x and y have different signs -> compare by signs\n")
        self._output_stream.write("@SP\n"
                                  "A=M\n"
                                  "M=-1\n"
                                  "@R14\n")  # R14 = M = (y >= 0)
    ...
    def _create_label(self, name: str, inc: bool = False) -> str:
        ...
        return f"{self._filename}.{self._func_name}.{name}_{self._label_counter}"
    ...
    def write_label(self, label: str) -> None:
        ...
        """
        self._output_stream.write(f"({self._filename}.{label})\n" if not self._func_name
                                  else f"({self._filename}.{self._func_name}${label})\n")
    ...
    def write_if(self, label: str) -> None:
        ...
        Args:
            label (str): the label to go to.
        """
        self._output_stream.write(f"// if condition above is TRUE -> got to {label}\n")
        self._decrease_sp(True)
        self._output_stream.write("D=M\n")
        self._output_stream.write(f"@{self._filename}.{label}\n" if not self._func_name
                                  else f"@{self._filename}.{self._func_name}${label}\n")
    ...
    def write_function(self, function_name: str, n_vars: int) -> None:
        ...
        # self.write_label(function_name)  # TODO check sharon implementation!
    ...
    def write_call(self, function_name: str, n_args: int) -> None:
        ...
        ret_label = self.ret_call_label(function_name)

        # push return_address
        self._output_stream.write(f"@{ret_label}\n"
                                  "D=A\n"
                                  "@SP\n"
                                  "A=M\n"
                                  "M=D\n")
        ...
        for tag in ("LCL", "ARG", "THIS", "THAT"):
            self._output_stream.write(f"@{tag}\n"
                                      "D=M\n"
                                      "@SP\n"
                                      "A=M\n"
                                      "M=D\n")
    ...
    def write_return(self) -> None:
        ...
        # frame = LCL.  R13 = address of LCL
        self._output_stream.write("@LCL\n"
                                  "D=M\n"
                                  "@R13\n"
                                  "M=D\n")

        # return_address = *(frame-5). R14 = return address
        self._output_stream.write("@5\n"
                                  "D=D-A\n"
                                  "A=D\n"
                                  "D=M\n"
    ...
    def bootstrap(self):
        self._output_stream.write(f"@{CodeWriter.SP}\n"
                                  f"D=A\n"
                                  f"@SP\n"
                                  f"M=D\n")
        self.write_call("Sys.init", 0)

    def ret_call_label(self, function_name):
        self.ret_count += 1
        return f"{function_name}$ret.{self.ret_count}"
